...
class SMSService {
  ...
  async sendOptOutConfirmation(providerPhone) {
    const message = "✅ You've been opted out from Gold Touch List notifications. Reply START to opt back in.";
    return await this.sendSMS(providerPhone, message);
  }
...
Thanks for using Gold Touch List!`;
  }
  ...
  async handleOptIn(phoneNumber) {
    ...
    try {
      ...
      await this.sendSMS(phoneNumber, "✅ You're now opted back in to Gold Touch List Lead notifications!");
    } catch (error) {
