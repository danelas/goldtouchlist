    ...
    try {
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are the Gold Touch List Lead Matching Agent. Your job is to analyze client inquiries and match them with the most suitable service providers based on location, service type, and other relevant factors. Always respond with valid JSON."
          },
          {
            role: "user",
            content: prompt
...
`;
  ...
  async generateTeaserEnhancement(leadData) {
    try {
      const prompt = `
As the Gold Touch List Lead Agent, enhance this lead summary for provider notifications:
...
`;
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are the Gold Touch List communication agent. Generate appropriate follow-up messages based on interaction context."
          },
          {
            role: "user",
            content: prompt
